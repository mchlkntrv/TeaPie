...
Requests generated by a `{% for %}` loop are named per iteration (e.g. `CreatePartner1`, `CreatePartner2`, ...) and are accessed the same way via `tp.Requests`/`tp.Responses` — see [Templating Loops](../templating-loops.md).
